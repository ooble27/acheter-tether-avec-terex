...
const PrivacyPage = () => {
  ...
  useEffect(() => {
    ...
    const setActive = (id: string) => {
      document.querySelectorAll('.pp-toc a').forEach((a) => {
        a.classList.toggle('pp-active', a.getAttribute('href') === `#${id}`);
      });
    };
  ...
  return (
    <div className="min-h-screen relative overflow-x-hidden" style={{ background: 'hsl(var(--terex-dark))', color: 'hsl(var(--foreground))' }}>
      <style>{`
        @keyframes pp-up { from { opacity: 0; transform: translateY(16px); } to { opacity: 1; transform: translateY(0); } }
        .pp-fade { animation: pp-up 0.7s cubic-bezier(0.22,1,0.36,1) both; }
        .pp-toc a.pp-active { color: #fff !important; background: hsl(var(--terex-accent) / 0.05) !important; }
        @media (max-width: 1100px) { .pp-vline { display: none !important; } }
      `}</style>

      {/* Vertical guide lines */}
      <div className="pp-vline" style={{ position: 'fixed', top: 0, bottom: 0, left: 'calc(50% - 560px)', width: 1, background: 'hsl(var(--terex-accent) / 0.05)', pointerEvents: 'none', zIndex: 0 }} />
      <div className="pp-vline" style={{ position: 'fixed', top: 0, bottom: 0, right: 'calc(50% - 560px)', width: 1, background: 'hsl(var(--terex-accent) / 0.05)', pointerEvents: 'none', zIndex: 0 }} />
      ...
      {/* Hero */}
      <section className="relative z-[1] pt-12 pb-8 md:pt-20 md:pb-12" style={{ borderBottom: '1px solid hsl(var(--terex-accent) / 0.07)' }}>
        <div className="pp-fade max-w-6xl mx-auto px-5 sm:px-6">
          <p style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.18em', color: 'hsl(var(--terex-accent) / 0.4)', margin: '0 0 14px' }}>Confidentialité</p>
          <h1 style={{ fontSize: 'clamp(1.9rem, 4vw, 2.6rem)', fontWeight: 800, letterSpacing: '-0.03em', lineHeight: 1.08, color: 'hsl(var(--foreground))', margin: '0 0 18px' }}>
            Politique de<br className="hidden md:block" /> Confidentialité
          </h1>
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2" style={{ fontSize: 12.5, color: 'hsl(var(--terex-accent) / 0.55)' }}>
            <span style={{ padding: '4px 12px', borderRadius: 999, background: 'hsl(var(--terex-darker))', border: '1px solid hsl(var(--terex-accent) / 0.07)' }}>RGPD &amp; Loi sénégalaise 2008-12</span>
      ...
      {/* Info bar */}
      <div className="relative z-[1]" style={{ background: 'hsl(var(--terex-accent) / 0.02)', borderBottom: '1px solid hsl(var(--terex-accent) / 0.07)' }}>
        <div className="max-w-6xl mx-auto px-5 sm:px-6 py-3.5 flex flex-wrap gap-2 items-center" style={{ fontSize: 12.5, color: 'hsl(var(--terex-accent) / 0.55)' }}>
          <span style={{ fontWeight: 600, color: 'hsl(var(--terex-accent) / 0.8)' }}>Protection de vos données :</span>
      ...
      <div className="relative z-[1] max-w-6xl mx-auto px-5 sm:px-6 flex gap-12 py-12 md:py-16">
        ...
        <aside className="hidden lg:block w-64 flex-shrink-0">
          <div className="sticky top-24 pp-toc">
            <p style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.18em', color: 'hsl(var(--terex-accent) / 0.4)', margin: '0 0 16px' }}>Table des matières</p>
            <nav className="space-y-0.5">
              {tocItems.map((item) => (
                <a
                  key={item.id}
                  href={`#${item.id}`}
                  className="block py-1.5 px-3 rounded-md transition-all leading-snug"
                  style={{ fontSize: 12, color: 'hsl(var(--terex-accent) / 0.55)' }}
                  onMouseEnter={(e) => { e.currentTarget.style.color = '#fff'; e.currentTarget.style.background = 'hsl(var(--terex-accent) / 0.04)'; }}
                  onMouseLeave={(e) => { e.currentTarget.style.color = 'hsl(var(--terex-accent) / 0.55)'; e.currentTarget.style.background = 'transparent'; }}
                >
                  {item.label}
                </a>
              ))}
        ...
        {/* Main content */}
        <main className="flex-1 min-w-0 max-w-[760px] space-y-14">

          {/* Section 1 */}
          <section id="introduction">
            <h2 className="mb-5 pb-3" style={{ fontSize: 'clamp(1.15rem, 2.4vw, 1.45rem)', fontWeight: 700, letterSpacing: '-0.02em', color: 'hsl(var(--foreground))', borderBottom: '1px solid hsl(var(--terex-accent) / 0.07)' }}>
              1. Introduction et Identité du Responsable de Traitement
            </h2>
            <div className="space-y-4" style={{ color: 'hsl(var(--terex-accent) / 0.55)', fontSize: 14.5, lineHeight: 1.75 }}>
          ...
          <section id="modifications">
            ...
            <div className="space-y-4" style={{ color: 'hsl(var(--terex-accent) / 0.55)', fontSize: 14.5, lineHeight: 1.75 }}>
              ...
              {/* Contact box */}
              <div className="space-y-3 mt-6" style={{ background: 'hsl(var(--terex-darker))', border: '1px solid hsl(var(--terex-accent) / 0.07)', borderRadius: 16, padding: 22 }}>
                <p style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'hsl(var(--terex-accent) / 0.8)', margin: 0 }}>Coordonnées du Responsable de Traitement</p>
                <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-2" style={{ fontSize: 14 }}>
                  <span style={{ color: 'hsl(var(--terex-accent) / 0.5)', fontWeight: 500 }}>Société</span>
                  <span>Terex — Teranga Exchange</span>
                  <span style={{ color: 'hsl(var(--terex-accent) / 0.5)', fontWeight: 500 }}>Adresse</span>
                  <span>Plateau, Avenue Léopold Sédar Senghor, Dakar, République du Sénégal</span>
                  <span style={{ color: 'hsl(var(--terex-accent) / 0.5)', fontWeight: 500 }}>E-mail général</span>
                  <a href="mailto:terangaexchange@gmail.com" style={{ color: 'hsl(var(--foreground))', textDecoration: 'underline' }}>terangaexchange@gmail.com</a>
                  <span style={{ color: 'hsl(var(--terex-accent) / 0.5)', fontWeight: 500 }}>E-mail DPO</span>
                  <a href="mailto:dpo@terex.sn" style={{ color: 'hsl(var(--foreground))', textDecoration: 'underline' }}>dpo@terex.sn</a>
                  <span style={{ color: 'hsl(var(--terex-accent) / 0.5)', fontWeight: 500 }}>Mise à jour</span>
                  <span>mars 2025</span>
                </div>
              </div>
            </div>
