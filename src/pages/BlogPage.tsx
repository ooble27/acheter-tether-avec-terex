  ...
  return (
    <div style={{ background: BG, minHeight: '100vh', color: 'hsl(var(--foreground))', position: 'relative', overflowX: 'hidden' }}>
      <style>{`
        ...
        .bp-tile:hover { transform: translateY(-3px); border-color: hsl(var(--terex-accent) / 0.16) !important; }
      ...
      <div className="bp-vline" style={{ position: 'fixed', top: 0, bottom: 0, left: 'calc(50% - 560px)', width: 1, background: 'hsl(var(--terex-accent) / 0.05)', pointerEvents: 'none', zIndex: 0 }} />
      <div className="bp-vline" style={{ position: 'fixed', top: 0, bottom: 0, right: 'calc(50% - 560px)', width: 1, background: 'hsl(var(--terex-accent) / 0.05)', pointerEvents: 'none', zIndex: 0 }} />
        ...
        <div className="bp-fade" style={{ maxWidth: 760 }}>
          ...
          <div style={{ position: 'relative', maxWidth: 380 }}>
            ...
            <input
              ...
              style={{ width: '100%', height: 48, borderRadius: 12, background: CARD, border: `1px solid ${BORDER}`, color: 'hsl(var(--foreground))', fontSize: 14, padding: '0 16px 0 44px', outline: 'none' }}
            />
          </div>
        </div>
      </header>
            ...
            <div className="bp-feat" style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr' }}>
              ...
              <div style={{ padding: 'clamp(28px, 4vw, 48px)', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                  <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', padding: '5px 11px', borderRadius: 999, background: 'hsl(var(--terex-accent))', color: 'hsl(var(--terex-accent-fg))' }}>À la une</span>
                  <span style={{ fontSize: 11, fontWeight: 500, padding: '5px 12px', borderRadius: 999, background: 'hsl(var(--terex-accent) / 0.06)', border: `1px solid ${BORDER}`, color: 'hsl(var(--terex-accent) / 0.7)' }}>{featured.category}</span>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 14, fontWeight: 700, color: 'hsl(var(--foreground))' }} className="bp-arrow">Lire l'article <ArrowRight size={16} /></span>
      ...
      <section className="bp-pad" style={{ maxWidth: 1120, margin: '0 auto', padding: '40px 32px 80px', position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 34 }}>
          ...
          <div style={{ flex: 1, height: 1, background: 'hsl(var(--terex-accent) / 0.06)' }} />
        </div>
            ...
            {gridArticles.map((a) => (
              <Link key={a.slug} to={`/blog/${a.slug}`} className="bp-tile" style={{ textDecoration: 'none', color: 'inherit', display: 'flex', flexDirection: 'column', background: CARD, border: `1px solid ${BORDER}`, borderRadius: 18, overflow: 'hidden' }}>
                ...
                <div style={{ padding: '22px 22px 20px', display: 'flex', flexDirection: 'column', flex: 1 }}>
                  <span style={{ alignSelf: 'flex-start', fontSize: 10.5, fontWeight: 500, padding: '4px 11px', borderRadius: 999, background: 'hsl(var(--terex-accent) / 0.06)', border: `1px solid ${BORDER}`, color: 'hsl(var(--terex-accent) / 0.65)', marginBottom: 14 }}>{a.category}</span>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 14, borderTop: `1px solid ${BORDER}` }}>
                    ...
                    <ArrowRight size={16} className="bp-arrow" color="hsl(var(--terex-accent) / 0.7)" />
                  </div>
                </div>
              </Link>
            ))}
      ...
      <section style={{ borderTop: `1px solid ${BORDER}`, position: 'relative', zIndex: 1 }}>
        <div className="bp-pad" style={{ maxWidth: 1120, margin: '0 auto', padding: '48px 32px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 20 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
            {RESOURCES.map((r) => (
              <Link key={r.to} to={r.to} className="bp-tile" style={{ textDecoration: 'none', color: 'hsl(var(--terex-accent) / 0.8)', border: `1px solid ${BORDER}`, borderRadius: 999, padding: '10px 18px', fontSize: 13.5, fontWeight: 500 }}>{r.label}</Link>
            ))}
          </div>
        </div>
      </section>
