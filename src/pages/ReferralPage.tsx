  ...
  return (
    <div style={{ background: BG, minHeight: '100vh', color: 'hsl(var(--foreground))', position: 'relative', overflowX: 'hidden' }}>
      <style>{`
        ...
        .rf-tile:hover { transform: translateY(-2px); border-color: hsl(var(--terex-accent) / 0.16) !important; }
      ...
      <div className="rf-vline" style={{ position: 'fixed', top: 0, bottom: 0, left: 'calc(50% - 560px)', width: 1, background: 'hsl(var(--terex-accent) / 0.05)', pointerEvents: 'none', zIndex: 0 }} />
      <div className="rf-vline" style={{ position: 'fixed', top: 0, bottom: 0, right: 'calc(50% - 560px)', width: 1, background: 'hsl(var(--terex-accent) / 0.05)', pointerEvents: 'none', zIndex: 0 }} />
      ...
      <div className="rf-pad" style={{ maxWidth: 1120, margin: '0 auto', padding: '32px 32px 96px', position: 'relative', zIndex: 1 }}>
        ...
        <div className="rf-dash" style={{ display: 'grid', gridTemplateColumns: '1fr 1.02fr', gap: 56, alignItems: 'center' }}>
          <div className="rf-fade">
            ...
            <p style={{ fontSize: 14, color: MUTED2, margin: 0 }}>
              Taux actuel{' '}
              {rateDisplay ? (
                <span style={{ color: 'hsl(var(--foreground))', fontWeight: 600 }}>{rateDisplay} CFA</span>
              ) : (
                <span className="rf-skel" style={{ display: 'inline-block', width: 70, height: 13, borderRadius: 6, background: 'hsl(var(--terex-accent) / 0.08)', verticalAlign: 'middle' }} />
              )}{' '}
              / USDT.
            </p>
          </div>
          ...
          <div className="rf-fade-2" style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 22, padding: 20 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 36, height: 36, borderRadius: 11, background: ICON_BG, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Gift size={18} color="hsl(var(--foreground))" strokeWidth={1.8} />
                </div>
              ...
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11, color: MUTED }}>
                <span style={{ position: 'relative', width: 7, height: 7 }}>
                  <span className="rf-live-dot" style={{ position: 'absolute', inset: 0, borderRadius: '50%', background: 'hsl(var(--terex-accent) / 0.55)' }} />
                </span>
                Actif
              </span>
            </div>
            ...
            <div style={{ background: CARD2, border: `1px solid ${BORDER}`, borderRadius: 16, padding: '16px 18px', marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                ...
                <button onClick={handleCopyCode} className="rf-cta" style={{ flexShrink: 0, background: 'hsl(var(--terex-accent))', color: 'hsl(var(--terex-accent-fg))', border: 'none', borderRadius: 10, height: 36, padding: '0 14px', fontSize: 12.5, fontWeight: 700, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 14 }}>
              {[
                ...
                <div key={label} style={{ background: CARD2, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '14px 12px' }}>
                  <Icon size={15} color="hsl(var(--terex-accent) / 0.55)" strokeWidth={1.8} style={{ marginBottom: 8 }} />
            ...
            <div style={{ background: CARD2, border: `1px solid ${BORDER}`, borderRadius: 16, padding: '6px 6px' }}>
              {INVITES.map((inv, i) => (
                <div key={inv.name} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 12px', borderTop: i === 0 ? 'none' : `1px solid ${BORDER}` }}>
                  <div style={{ width: 34, height: 34, borderRadius: 11, background: ICON_BG, border: `1px solid hsl(var(--terex-accent) / 0.08)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>{inv.initials}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 600 }}>{inv.name}</div>
                    <div style={{ fontSize: 11, color: MUTED2 }}>{inv.when}</div>
                  </div>
                  <span style={{ fontSize: 11, fontWeight: 600, color: inv.status === 'Actif' ? 'hsl(var(--terex-accent) / 0.55)' : MUTED, background: inv.status === 'Actif' ? 'rgba(74,222,128,0.1)' : ICON_BG, borderRadius: 999, padding: '4px 10px' }}>{inv.status}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="rf-fade" style={{ marginTop: 56, border: `1px solid ${BORDER}`, background: CARD, borderRadius: 20, padding: 'clamp(24px,4vw,36px)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', border: `1px solid ${BORDER}`, borderRadius: 16, padding: '20px 22px', background: 'hsl(var(--terex-accent) / 0.02)', marginBottom: 16 }}>
            ...
            <button
              ...
              className="rf-cta"
              style={{ background: 'hsl(var(--terex-accent))', color: 'hsl(var(--terex-accent-fg))', border: 'none', borderRadius: 12, height: 44, padding: '0 20px', fontSize: 14.5, fontWeight: 700, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8 }}
            >
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            <div style={{ flex: '1 1 240px', minWidth: 0, border: `1px solid ${BORDER}`, borderRadius: 12, height: 44, padding: '0 16px', display: 'flex', alignItems: 'center', background: BG, fontSize: 13.5, color: 'hsl(var(--terex-accent) / 0.7)', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>
              {referralLink}
            </div>
            <button
              onClick={handleCopyLink}
              className="rf-cta"
              style={{ background: 'hsl(var(--terex-gray))', color: 'hsl(var(--foreground))', border: `1px solid hsl(var(--terex-accent) / 0.08)`, borderRadius: 12, height: 44, padding: '0 18px', fontSize: 14, fontWeight: 600, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8 }}
            >
        ...
        <div style={{ marginTop: 64 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 28 }}>
            <Award size={22} color="hsl(var(--terex-accent) / 0.9)" strokeWidth={1.8} />
            ...
            <div className="rf-reward-feat rf-tile" style={{ gridColumn: '1 / 2', gridRow: '1 / 3', border: `1px solid ${BORDER}`, background: CARD, borderRadius: 22, padding: '30px 28px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              <div>
                <div style={{ width: 46, height: 46, borderRadius: 14, background: ICON_BG, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
                  <Users size={22} color="hsl(var(--foreground))" strokeWidth={1.8} />
                </div>
              ...
              <div style={{ marginTop: 24, background: CARD2, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 700, color: 'hsl(var(--terex-accent) / 0.55)' }}>+50 USDT</span>
              </div>
            </div>
            ...
            {[
              ...
              <div key={kicker} className="rf-tile" style={{ gridColumn: '2 / 4', border: `1px solid ${BORDER}`, background: CARD, borderRadius: 20, padding: '24px 24px', display: 'flex', alignItems: 'center', gap: 20 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ fontSize: 13.5, color: 'hsl(var(--terex-accent) / 0.5)', lineHeight: 1.5, margin: 0 }}>{sub}</p>
                </div>
            ...
            {STEPS.map(({ n, title, desc }, i) => (
              <div key={n} className="rf-tile" style={{ position: 'relative', border: `1px solid ${BORDER}`, background: CARD, borderRadius: 20, padding: '28px 24px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                  <div style={{ width: 40, height: 40, borderRadius: 12, background: 'hsl(var(--terex-accent))', color: 'hsl(var(--terex-accent-fg))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 800, flexShrink: 0 }}>{n}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }} className="rf-steps">
            ...
            <div className="rf-tile" style={{ border: `1px solid ${BORDER}`, background: CARD, borderRadius: 20, padding: '28px 26px' }}>
              ...
              <p style={{ fontSize: 'clamp(2rem,5vw,2.6rem)', fontWeight: 800, letterSpacing: '-0.03em', lineHeight: 1, margin: 0 }}>
                0 <span style={{ fontSize: '0.45em', fontWeight: 600, color: 'hsl(var(--terex-accent) / 0.45)' }}>CFA</span>
              </p>
            </div>
          </div>
        </div>
