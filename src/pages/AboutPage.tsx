  ...
  return (
    <div style={{ background: BG, minHeight: '100vh', color: 'hsl(var(--foreground))', position: 'relative', overflowX: 'hidden' }}>
      <style>{`
        ...
        .ab-tile:hover { transform: translateY(-3px); border-color: hsl(var(--terex-accent) / 0.16) !important; }
      ...
      <div className="ab-vline" style={{ position: 'fixed', top: 0, bottom: 0, left: 'calc(50% - 560px)', width: 1, background: 'hsl(var(--terex-accent) / 0.05)', pointerEvents: 'none', zIndex: 0 }} />
      <div className="ab-vline" style={{ position: 'fixed', top: 0, bottom: 0, right: 'calc(50% - 560px)', width: 1, background: 'hsl(var(--terex-accent) / 0.05)', pointerEvents: 'none', zIndex: 0 }} />
      ...
      <header className="ab-pad" style={{ maxWidth: 1120, margin: '0 auto', padding: '80px 32px 72px', position: 'relative', zIndex: 1 }}>
        <div className="ab-hero" style={{ display: 'grid', gridTemplateColumns: '1.05fr 0.95fr', gap: 56, alignItems: 'center' }}>
          <div className="ab-fade">
            ...
            <button onClick={() => navigate('/auth')} className="ab-cta" style={{ background: 'hsl(var(--terex-accent))', color: 'hsl(var(--terex-accent-fg))', border: 'none', borderRadius: 12, height: 50, padding: '0 26px', fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 22, padding: 18 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 8px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ position: 'relative', width: 8, height: 8 }}>
                    <span className="ab-live-dot" style={{ position: 'absolute', inset: 0, borderRadius: '50%', background: 'hsl(var(--terex-accent))' }} />
                  </span>
                  <span style={{ fontSize: 12, fontWeight: 600, color: 'hsl(var(--terex-accent) / 0.7)' }}>Activité en direct</span>
                </div>
              ...
              <div style={{ height: 280, overflow: 'hidden', position: 'relative', WebkitMaskImage: 'linear-gradient(transparent, #000 12%, #000 88%, transparent)', maskImage: 'linear-gradient(transparent, #000 12%, #000 88%, transparent)' }}>
                <div className="ab-feed-track" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {[...FEED, ...FEED].map((f, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 12px', borderRadius: 14, background: 'hsl(var(--terex-darker))', border: `1px solid ${BORDER}` }}>
                      <div style={{ width: 38, height: 38, borderRadius: 11, background: ICON_BG, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        {f.type === 'buy' ? <Coins size={18} color="hsl(var(--terex-accent) / 0.85)" /> : <HandCoins size={18} color="hsl(var(--terex-accent) / 0.85)" />}
                      </div>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        ...
                        <div style={{ fontSize: 11, color: MUTED2, display: 'flex', alignItems: 'center', gap: 4, justifyContent: 'flex-end' }}>
                          <CheckCircle2 size={11} color="hsl(var(--terex-accent) / 0.5)" /> {f.usdt}
                        </div>
                      </div>
                    </div>
                  ))}
      ...
      <section style={{ borderTop: `1px solid ${BORDER}`, position: 'relative', zIndex: 1 }}>
        <div className="ab-pad" style={{ maxWidth: 1120, margin: '0 auto', padding: '88px 32px' }}>
          ...
          <p style={{ fontSize: 'clamp(1.6rem, 3.4vw, 2.5rem)', fontWeight: 600, letterSpacing: '-0.02em', lineHeight: 1.35, margin: 0, maxWidth: 920 }}>
            Nous croyons que chaque Africain mérite un accès <span style={{ color: 'hsl(var(--foreground))' }}>simple</span> et <span style={{ color: 'hsl(var(--foreground))' }}>sécurisé</span> aux services financiers numériques. <span style={{ color: MUTED2 }}>Terex relie les crypto-monnaies aux monnaies locales pour libérer les échanges, du smartphone le plus modeste à l'entreprise la plus exigeante.</span>
          </p>
        </div>
      </section>
            ...
            <div className="ab-bento-feat ab-tile" style={{ gridColumn: '1 / 2', gridRow: '1 / 3', border: `1px solid ${BORDER}`, borderRadius: 22, padding: '30px 28px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative', overflow: 'hidden' }}>
              <div>
                <div style={{ width: 46, height: 46, borderRadius: 14, background: ICON_BG, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
                  <Shield size={22} color="hsl(var(--foreground))" strokeWidth={1.8} />
                </div>
              ...
              <div style={{ position: 'relative', height: 96, display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 24 }}>
                ...
                <div style={{ width: 56, height: 56, borderRadius: 16, background: 'hsl(var(--terex-darker))', border: `1px solid hsl(var(--terex-accent) / 0.12)`, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1 }}>
                  <Lock size={22} color="hsl(var(--foreground))" />
                </div>
              </div>
            </div>
        ...
        <div className="ab-pad" style={{ maxWidth: 1120, margin: '0 auto', padding: '80px 32px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 56, alignItems: 'center' }} className="ab-hero">
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
              {COUNTRIES.map((c) => (
                <div key={c} className="ab-tile" style={{ border: `1px solid ${BORDER}`, borderRadius: 999, padding: '11px 18px', fontSize: 14, fontWeight: 500, color: 'hsl(var(--terex-accent) / 0.8)' }}>{c}</div>
              ))}
            </div>
          </div>
        </div>
          ...
          <div style={{ position: 'relative', paddingLeft: 28, maxWidth: 720 }}>
            ...
            {MILESTONES.map((m, i) => (
              <div key={i} style={{ position: 'relative', paddingBottom: i < MILESTONES.length - 1 ? 30 : 0 }}>
                <span style={{ position: 'absolute', left: -28, top: 4, width: 11, height: 11, borderRadius: '50%', background: i === 0 ? '#fff' : 'hsl(var(--terex-darker))', border: `1px solid ${i === 0 ? '#fff' : 'hsl(var(--terex-accent) / 0.25)'}` }} />
          ...
          <div className="ab-grid3" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
            {TEAM.map((m) => (
              <div key={m.name} className="ab-tile" style={{ border: `1px solid ${BORDER}`, borderRadius: 20, padding: '28px 26px' }}>
                <div style={{ width: 54, height: 54, borderRadius: 16, background: 'hsl(var(--terex-darker))', border: `1px solid hsl(var(--terex-accent) / 0.1)`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 18, fontSize: 17, fontWeight: 700, letterSpacing: '0.02em' }}>{m.initials}</div>
                ...
                <p style={{ fontSize: 13.5, color: 'hsl(var(--terex-accent) / 0.5)', lineHeight: 1.65, margin: 0 }}>{m.bio}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ borderTop: `1px solid ${BORDER}`, position: 'relative', zIndex: 1, overflow: 'hidden' }}>
        <div className="ab-pad" style={{ maxWidth: 1120, margin: '0 auto', padding: '96px 32px', textAlign: 'center', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <button onClick={() => navigate('/auth')} className="ab-cta" style={{ background: 'hsl(var(--terex-accent))', color: 'hsl(var(--terex-accent-fg))', border: 'none', borderRadius: 12, height: 52, padding: '0 32px', fontSize: 16, fontWeight: 700, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
              Créer un compte <ArrowRight size={17} />
            </button>
            <button onClick={() => navigate('/careers')} style={{ background: 'hsl(var(--terex-gray))', color: 'hsl(var(--foreground))', border: '1px solid hsl(var(--terex-accent) / 0.08)', borderRadius: 12, height: 52, padding: '0 28px', fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>
              Nous rejoindre
            </button>
          </div>
        </div>
