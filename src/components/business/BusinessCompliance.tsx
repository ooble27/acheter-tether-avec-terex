...
import {
  ...
  FileText, TrendingUp, Clock, AlertCircle, Plus, Send,
} from 'lucide-react';
...
const LEVEL_INFO = [
  { n: 1, label: 'Basique',  limit: '5 000',    sub: 'USDT/mois', state: 'done'   as const },
  { n: 2, label: 'Vérifié', limit: '50 000',   sub: 'USDT/mois', state: 'active' as const },
  { n: 3, label: 'Avancé',  limit: '200 000',  sub: 'USDT/mois', state: 'locked' as const },
  { n: 4, label: 'Premium', limit: 'Illimitée',sub: '',          state: 'locked' as const },
];

const INITIAL_DOCS: Doc[] = [
  // Niveau 1 — identité entreprise (marché africain)
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 0', borderBottom: `1px solid ${C.bds}` }}>
        <div style={{ width: 34, height: 34, borderRadius: 9, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: s.bg, border: `1px solid ${s.border}` }}>
          {doc.status === 'verified'  && <Check size={15} color={C.teal} strokeWidth={2.5} />}
          {doc.status === 'submitted' && <Clock size={14} color={C.teal} />}
          {doc.status === 'required'  && <FileText size={14} color={C.t3} />}
        ...
        {(doc.status === 'required' || doc.status === 'expired') && (
          <button onClick={() => setModal(true)}
            style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 13px', borderRadius: 8, fontSize: 12, fontWeight: 500, border: `1px solid ${C.bds}`, background: 'transparent', color: C.t2, cursor: 'pointer', fontFamily: FONT, transition: 'all 0.13s', flexShrink: 0 }}
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.72)', backdropFilter: 'blur(6px)' }}
          ...
          <div style={{ background: C.l1, border: `1px solid ${C.bd}`, borderRadius: 18, padding: 28, width: 420, boxShadow: '0 28px 70px rgba(0,0,0,0.65)', fontFamily: FONT }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <TealBtn onClick={() => { onSubmit(doc.id); setModal(false); }} style={{ flex: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7 }}>
...
function LimitRequestPage({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div style={{ fontFamily: FONT, maxWidth: 1040, margin: '0 auto', paddingBottom: 48 }}>
      ...
      {/* Hero */}
      <div style={{ background: 'linear-gradient(135deg, #1e1e1e 0%, #181818 60%, #141414 100%)', border: `1px solid ${C.bds}`, borderRadius: 16, padding: '22px 28px', marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              <h2 style={{ color: C.t1, fontSize: 19, fontWeight: 700, letterSpacing: '-0.025em', margin: 0 }}>Demande d'augmentation de limite</h2>
              <p style={{ color: C.t3, fontSize: 12, margin: '4px 0 0' }}>Votre demande sera examinée sous 24–48h ouvrées</p>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <GhostBtn onClick={onBack} style={{ height: 40 }}>Annuler</GhostBtn>
            {!sent && (
              <TealBtn onClick={() => { if (canSend) setSent(true); }} disabled={!canSend}
                style={{ height: 40, display: 'flex', alignItems: 'center', gap: 7 }}>
      ...
      {sent ? (
        <div style={{ ...card, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '48px 32px', gap: 16 }}>
          <div style={{ width: 56, height: 56, borderRadius: '50%', background: C.tealT, border: `1px solid ${C.tealB}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Check size={24} color={C.teal} strokeWidth={2.5} />
          </div>
          ...
          <TealBtn onClick={onBack} style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 7 }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2" style={{ gap: 16, alignItems: 'start' }}>
            ...
            <div style={card}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div>
                  ...
                  <input value={sector} onChange={e => setSector(e.target.value)} placeholder="Ex : Import/export, Commerce de gros…" style={inp} />
                </div>
                <div>
                  <label style={lbl}>Justification détaillée</label>
                  <textarea value={justification} onChange={e => setJust(e.target.value)}
                    placeholder="Décrivez votre activité, vos fournisseurs principaux et la raison de cette demande d'augmentation…"
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={card}>
              <p style={{ fontSize: 10, fontWeight: 700, color: C.t3, textTransform: 'uppercase', letterSpacing: '0.10em', margin: '0 0 16px' }}>Votre situation actuelle</p>
              {[
                { label: 'Niveau', value: 'Niveau 1 — Basique' },
                { label: 'Limite mensuelle', value: '5 000 USDT' },
                { label: 'Limite par transaction', value: '2 000 USDT' },
              ].map(r => (
                <div key={r.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: `1px solid ${C.bds}` }}>
            ...
            <div style={{ ...card, background: C.tealT, border: `1px solid ${C.tealB}` }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                <ShieldCheck size={18} color={C.teal} style={{ flexShrink: 0, marginTop: 1 }} />
                <div>
                  <p style={{ fontSize: 13, fontWeight: 600, color: C.teal, margin: '0 0 6px' }}>Processus de validation</p>
                  <ul style={{ color: C.t2, fontSize: 12, margin: 0, padding: '0 0 0 16px', lineHeight: 1.8 }}>
                    <li>Vérification de votre dossier KYC</li>
                    <li>Analyse du volume mensuel déclaré</li>
                    <li>Validation par l'équipe conformité</li>
                    <li>Réponse par email sous 24–48h</li>
                  </ul>
                </div>
              </div>
            </div>
            ...
            <div style={card}>
              <p style={{ fontSize: 10, fontWeight: 700, color: C.t3, textTransform: 'uppercase', letterSpacing: '0.10em', margin: '0 0 14px' }}>Documents complémentaires</p>
              <p style={{ color: C.t3, fontSize: 12, margin: '0 0 12px', lineHeight: 1.5 }}>
                Des justificatifs supplémentaires pourront être demandés après examen de votre dossier.
              </p>
              <div style={{ display: 'flex', gap: 6 }}>
                <button style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '8px 0', borderRadius: 9, border: `1px dashed ${C.bds}`, background: 'transparent', color: C.t3, fontSize: 12, cursor: 'pointer', fontFamily: FONT, transition: 'all 0.13s' }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = C.teal; e.currentTarget.style.color = C.teal; }}
...
export function BusinessCompliance({ user: _user }: { user: { email: string; name: string; id?: string } | null }) {
  ...
  const level3 = docs.filter(d => d.level === 3);

  const l2Done = level2.filter(d => d.status !== 'required').length;
  const l2Pct  = Math.round((l2Done / level2.length) * 100);
  ...
  return (
    <div style={{ fontFamily: FONT, color: C.t1 }}>
      ...
      {/* ── Layout 2 colonnes comme Trésorerie ── */}
      <div className="grid grid-cols-1 lg:grid-cols-[3fr_2fr]" style={{ gap: 14, alignItems: 'start' }}>

        {/* ══ COLONNE GAUCHE ══════════════════════════════════════════ */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>

          {/* Hero — statut conformité (pas en haut de page, dans la colonne gauche) */}
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 22 }}>
              ...
              <div style={{ padding: '4px 11px', background: C.tealT, border: `1px solid ${C.tealB}`, borderRadius: 20, fontSize: 11, color: C.teal, fontWeight: 600 }}>
                En cours de vérification
              </div>
            </div>

            <div style={{ marginBottom: 22 }}>
              <div style={{ fontSize: 11, color: C.t3, marginBottom: 6 }}>Niveau actuel</div>
              <div style={{ fontSize: 36, fontWeight: 700, fontFamily: MONO, letterSpacing: '-0.04em', lineHeight: 1, color: C.t1 }}>
                5 000
                <span style={{ fontSize: 16, color: C.t3, fontWeight: 400, marginLeft: 8, letterSpacing: 0 }}>USDT/mois</span>
              </div>
              <div style={{ fontSize: 13, color: C.t2, marginTop: 6 }}>Niveau 1 — Basique</div>
            </div>

            <div style={{ display: 'flex', gap: 24, marginBottom: 24 }}>
              <div style={{ paddingRight: 24, borderRight: `1px solid ${C.bds}` }}>
                <div style={{ fontSize: 10, color: C.t3, marginBottom: 3 }}>Documents vérifiés</div>
                <div style={{ fontSize: 18, fontWeight: 700, color: C.teal, fontFamily: MONO }}>{totalDone}</div>
              </div>
              <div style={{ paddingRight: 24, borderRight: `1px solid ${C.bds}` }}>
                <div style={{ fontSize: 10, color: C.t3, marginBottom: 3 }}>En vérification</div>
              ...
              <div>
                ...
                <div style={{ fontSize: 18, fontWeight: 700, color: C.t2, fontFamily: MONO }}>{totalReq}</div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button
                ...
                <ShieldCheck size={13} /> Niveau suivant
              </button>
            </div>
          </div>
          ...
          <div style={cardSt}>
            <div style={{ padding: '16px 22px', borderBottom: `1px solid ${C.bds}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: C.t1 }}>Niveau 1 — Identité entreprise</span>
                <span style={{ fontSize: 10, color: C.teal, background: C.tealT, border: `1px solid ${C.tealB}`, padding: '2px 8px', borderRadius: 10, fontFamily: MONO }}>5 000 USDT/mois</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                <Check size={12} color={C.teal} />
                <span style={{ fontSize: 11, color: C.teal, fontWeight: 600 }}>Complété</span>
              </div>
            </div>
            <div style={{ padding: '0 22px' }}>
              {level1.map(d => <DocItem key={d.id} doc={d} onSubmit={handleSubmitDoc} />)}
            </div>
          </div>
          ...
          <div style={cardSt}>
            <div style={{ padding: '16px 22px', borderBottom: `1px solid ${C.bds}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: C.t1 }}>Niveau 2 — Dirigeant</span>
                <span style={{ fontSize: 10, color: C.t3, background: 'rgba(255,255,255,0.04)', border: `1px solid ${C.bds}`, padding: '2px 8px', borderRadius: 10, fontFamily: MONO }}>50 000 USDT/mois</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ height: 3, width: 80, background: C.l3, borderRadius: 2, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${l2Pct}%`, background: C.teal, borderRadius: 2, transition: 'width 0.4s' }} />
                </div>
          ...
          {/* Documents — Niveau 3 */}
          <div style={{ ...cardSt, opacity: 0.65 }}>
            <div style={{ padding: '16px 22px', borderBottom: `1px solid ${C.bds}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: C.t2 }}>Niveau 3 — Structure</span>
                <span style={{ fontSize: 10, color: C.t3, background: 'rgba(255,255,255,0.04)', border: `1px solid ${C.bds}`, padding: '2px 8px', borderRadius: 10, fontFamily: MONO }}>200 000 USDT/mois</span>
              </div>
              <span style={{ fontSize: 11, color: C.t3 }}>Disponible après Niveau 2</span>
            </div>
            <div style={{ padding: '0 22px' }}>
              {level3.map(d => <DocItem key={d.id} doc={d} onSubmit={handleSubmitDoc} />)}
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={cardSt}>
            <div style={{ padding: '16px 20px', borderBottom: `1px solid ${C.bds}` }}>
              <span style={{ fontSize: 10, fontWeight: 700, color: C.t3, textTransform: 'uppercase', letterSpacing: '0.10em' }}>Niveaux de vérification</span>
            </div>
            <div style={{ padding: '8px 0' }}>
              {LEVEL_INFO.map((lv, i) => {
                ...
                return (
                  <div key={lv.n} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 20px', borderBottom: i < LEVEL_INFO.length - 1 ? `1px solid ${C.bds}` : 'none', opacity: locked ? 0.45 : 1 }}>
                    <div style={{ width: 32, height: 32, borderRadius: 10, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: done ? C.tealT : active ? 'rgba(255,255,255,0.06)' : C.l2, border: `1px solid ${done ? C.tealB : active ? 'rgba(255,255,255,0.12)' : C.bds}` }}>
                      {done   && <Check size={14} color={C.teal} strokeWidth={2.5} />}
                      {active && <div style={{ width: 8, height: 8, borderRadius: '50%', border: `2px solid ${C.teal}` }} />}
                      {locked && <div style={{ width: 8, height: 8, borderRadius: '50%', background: C.t3 }} />}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12.5, fontWeight: 600, color: active ? C.teal : done ? C.t1 : C.t2 }}>
                        Niveau {lv.n} — {lv.label}
                      </div>
                      <div style={{ fontSize: 11, color: C.t3, marginTop: 2, fontFamily: MONO }}>
                        {lv.limit}{lv.sub && ` ${lv.sub}`}
                      </div>
                    </div>
                    {active && <ChevronRight size={14} color={C.teal} />}
                  </div>
                );
              })}
            </div>
          ...
          <div style={cardSt}>
            ...
            <div style={{ padding: '8px 0' }}>
              {[
                ...
                { icon: ShieldCheck, label: 'Politique de conformité',   sub: 'Règlement intérieur',   action: () => {} },
              ].map((a, i, arr) => {
                const Icon = a.icon;
                return (
                  <button key={a.label} onClick={a.action}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 13, padding: '13px 20px', borderBottom: i < arr.length - 1 ? `1px solid ${C.bds}` : 'none', background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: FONT, textAlign: 'left', transition: 'background 0.12s' }}
                    onMouseEnter={e => (e.currentTarget.style.background = 'rgba(255,255,255,0.03)')}
                    onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                    <div style={{ width: 32, height: 32, borderRadius: 9, background: C.l2, border: `1px solid ${C.bds}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <Icon size={14} color={C.t3} />
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12.5, fontWeight: 500, color: C.t1 }}>{a.label}</div>
          ...
          {/* Expiration */}
          <div style={{ ...cardSt, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 10 }}>
