  ...
  return (
    <button
      ...
      style={{
        ...
        background: active ? 'hsl(var(--terex-accent) / 0.06)' : hov ? C.l2 : 'transparent',
...
function Avatar({ name, size = 36 }: { name: string; size?: number }) {
  return (
    <div style={{
      ...
      background: 'hsl(var(--terex-accent) / 0.18)', color: C.teal,
      display: 'flex', alignItems: 'center', justifyContent: 'center',
            ...
            <Card>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', background: C.l2, border: `1px solid ${C.bds}`, borderRadius: 10, marginBottom: 14 }}>
                ...
                <div style={{ textAlign: 'center' }}>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', background: rateLoading ? C.t3 : C.teal, margin: '0 auto 2px', boxShadow: rateLoading ? 'none' : `0 0 0 3px hsl(var(--terex-accent) / 0.15)` }} />
                  ...
                  {filteredSuppliers.map(s => {
                    ...
                    return (
                      <div key={s.id} onClick={() => setSelectedSupplier(s.id)} style={{
                        ...
                        background: isSelected ? 'hsl(var(--terex-accent) / 0.06)' : C.l2,
              ...
              <div style={{ border: `1px solid ${C.bds}`, borderRadius: 10, overflow: 'hidden' }}>
                {step3Rows.map((r, i) => (
                  <div key={i} style={{
                    ...
                    background: (r as any).bold ? 'hsl(var(--terex-accent) / 0.05)' : i % 2 === 0 ? 'transparent' : 'hsl(var(--terex-accent) / 0.01)',
                  }}>
        ...
        <Card style={{ padding: '56px 40px', textAlign: 'center' }}>
          <div style={{ marginBottom: 32 }}>
            <svg width="120" height="100" viewBox="0 0 120 100" fill="none" style={{ margin: '0 auto', display: 'block' }}>
              <circle cx="60" cy="50" r="38" stroke="hsl(var(--terex-accent) / 0.15)" strokeWidth="2"/>
              <circle cx="60" cy="50" r="38" stroke={C.teal} strokeWidth="2" strokeDasharray="60 180" strokeLinecap="round" transform="rotate(-90 60 50)"/>
              <circle cx="60" cy="50" r="26" fill="hsl(var(--terex-accent) / 0.08)" stroke={C.tealB} strokeWidth="1.5"/>
              <path d="M47 50 L56 59 L74 42" stroke={C.teal} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
              <circle cx="18" cy="20" r="7" fill="hsl(var(--terex-accent) / 0.1)" stroke={C.tealB} strokeWidth="1"/>
              <circle cx="102" cy="20" r="5" fill="hsl(var(--terex-accent) / 0.07)" stroke={C.tealB} strokeWidth="1"/>
              <circle cx="14" cy="72" r="4" fill="hsl(var(--terex-accent) / 0.07)" stroke={C.tealB} strokeWidth="1"/>
              <circle cx="106" cy="72" r="6" fill="hsl(var(--terex-accent) / 0.1)" stroke={C.tealB} strokeWidth="1"/>
