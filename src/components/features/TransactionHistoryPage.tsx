  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '32px 20px 120px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '28px', gap: '16px', flexWrap: 'wrap' }}>
        <div>
          <p style={{ color: 'hsl(var(--muted-foreground))', fontSize: '12px', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 4px' }}>Compte</p>
          <h1 style={{ color: 'hsl(var(--foreground))', fontSize: '26px', fontWeight: 700, margin: 0, letterSpacing: '-0.4px' }}>Historique</h1>
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
          <button
            onClick={handleRefresh}
            disabled={loading}
            style={{ display: 'flex', alignItems: 'center', gap: '6px', background: CARD, border: `1px solid ${BORDER}`, borderRadius: '12px', padding: '10px 16px', color: 'hsl(var(--foreground))', fontSize: '13px', fontWeight: 500, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.6 : 1 }}
          >
          ...
          <button
            disabled
            style={{ display: 'flex', alignItems: 'center', gap: '6px', background: CARD, border: `1px solid ${BORDER}`, borderRadius: '12px', padding: '10px 16px', color: 'hsl(var(--muted-foreground))', fontSize: '13px', fontWeight: 500, cursor: 'not-allowed' }}
          >
            <Download size={14} />
            Exporter
          </button>
        ...
        {stats.map(({ label, value, Icon }) => (
          <div key={label} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: '16px', padding: '16px 18px', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              ...
              <p style={{ color: 'hsl(var(--foreground))', fontSize: '22px', fontWeight: 700, margin: 0, letterSpacing: '-0.5px' }}>{value}</p>
            </div>
          </div>
        ))}
      </div>
