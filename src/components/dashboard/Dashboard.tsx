...
export function Dashboard({ user, onLogout }: DashboardProps) {
  ...
  return (
    <TransactionProvider>
      <SidebarProvider>
        <div className={`flex w-full bg-terex-dark ${isMobile ? 'min-h-screen' : 'min-h-screen'}`} style={{ backgroundColor: '#1a1a1a' }}>
          <AppSidebar 
            activeSection={activeSection}
            setActiveSection={setActiveSection}
            onLogout={handleLogout}
          />
          <main className={`flex-1 ${isMobile ? 'p-4 pt-16' : 'p-6'} relative`} style={{ backgroundColor: '#1a1a1a' }}>
            <MobileMenu 
              activeSection={activeSection}
              setActiveSection={setActiveSection}
              onLogout={handleLogout}
