...
export function AppSidebar({ activeSection, setActiveSection, onLogout }: AppSidebarProps) {
  ...
  return (
    <Sidebar className="bg-sidebar border-r border-sidebar-border shadow-2xl fixed left-0 top-0 h-screen z-50">
      <AppSidebarContent 
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <Button 
          variant="ghost" 
          size="icon"
          className="md:hidden fixed top-4 left-4 z-50 bg-sidebar/95 backdrop-blur-sm border border-sidebar-border text-sidebar-foreground hover:bg-sidebar-accent/80 shadow-lg rounded-xl w-12 h-12"
        >
      ...
      <SheetContent 
        side="left" 
        className="w-80 bg-sidebar border-r border-sidebar-border p-0 shadow-2xl fixed"
        style={{ 
          height: '100dvh',
          maxHeight: '100dvh',
          position: 'fixed',
