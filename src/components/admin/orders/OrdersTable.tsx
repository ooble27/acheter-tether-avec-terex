...
export function OrdersTable({ orders, onStatusUpdate }: OrdersTableProps) {
  ...
  const getPaymentStatusBadge = (paymentStatus?: string) => {
    ...
    return (
      <Badge 
        variant="outline" 
        className={statusStyles[paymentStatus as keyof typeof statusStyles] || 'bg-gray-500/10 text-gray-500'}
      >
  ...
  if (orders.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-400">Il n'y a aucune commande à afficher pour le moment.</p>
      </div>
    );
  }
          ...
          <div className="p-6">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
              ...
              <div className="flex items-center space-x-4">
                <div className="w-12 h-12 bg-terex-accent/10 rounded-lg flex items-center justify-center">
                  <DollarSign className="w-6 h-6 text-terex-accent" />
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-4 mt-1 text-sm text-gray-400">
                    ...
                    <span>•</span>
                    <span>{order.type === 'transfer' ? 'Transfert' : order.type === 'buy' ? 'Achat' : 'Vente'}</span>
                  </div>
                </div>
              </div>
          ...
          {expandedOrder === order.id && (
            <div className="border-t border-terex-gray/50 bg-terex-gray/20 p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                  <h4 className="text-sm font-medium text-gray-400 mb-2">Détails de la transaction</h4>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                    ...
                    {order.wallet_address && (
                      <div>
                        <span className="text-gray-400 block mb-1">Adresse wallet:</span>
