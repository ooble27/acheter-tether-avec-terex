...
import type { OrderStatus } from '@/hooks/useEnhancedOrders';

export function EnhancedOrdersAdmin() {
  const { orders, loading, updateOrderStatus, sendOrderEmail } = useEnhancedOrders();
  ...
  const handleSendEmail = async (orderId: string, orderData: any, emailType: 'confirmation' | 'processing' | 'completed' | 'cancelled') => {
    setSendingEmails(prev => [...prev, orderId]);
    await sendOrderEmail(orderId, orderData, emailType);
    setSendingEmails(prev => prev.filter(id => id !== orderId));
  };
    ...
    <div className="min-h-screen bg-terex-dark p-4">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-400">Gérez toutes les commandes avec notifications automatiques</p>
        </div>
...
interface EnhancedOrdersListProps {
  ...
  onSendEmail: (orderId: string, orderData: any, emailType: 'confirmation' | 'processing' | 'completed' | 'cancelled') => void;
  sendingEmails: string[];
  showActions: boolean;
}

function EnhancedOrdersList({ orders, onStatusUpdate, onSendEmail, sendingEmails, showActions }: EnhancedOrdersListProps) {
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending':
        return <Badge variant="outline" className="text-yellow-500 border-yellow-500">En attente</Badge>;
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
              ...
              {showActions && (
                <div className="flex flex-wrap gap-2">
                  ...
                  {/* Actions d'envoi d'email manuel */}
                  <div className="flex flex-wrap gap-1">
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => onSendEmail(order.id, order, 'confirmation')}
                      disabled={sendingEmails.includes(order.id)}
                      className="border-gray-600 text-gray-300 hover:bg-gray-700"
                    >
                      {sendingEmails.includes(order.id) ? (
                    ...
                    {order.status === 'processing' && (
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => onSendEmail(order.id, order, 'processing')}
                        disabled={sendingEmails.includes(order.id)}
                    ...
                    {order.status === 'completed' && (
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => onSendEmail(order.id, order, 'completed')}
                        disabled={sendingEmails.includes(order.id)}
